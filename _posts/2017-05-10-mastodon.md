...
Mastodonブームをながめつつ、自分でOStatus実装を書いてみようかと思っていたが、思ったよりずっと大変そうであることを知る。いずれにしてもMastodon管理者もやりたいし、現物で動作確認もしたいので、自前のMastodonサーバをたててみた。

自分専用インスタンスとして[https://sandbox.skoji.jp/](https://sandbox.skoji.jp)をたてた。

設置時の基本的な情報ソースは、公式の[Docker guide](https://github.com/tootsuite/documentation/blob/master/Running-Mastodon/Docker-Guide.md)、[Production guide](https://github.com/tootsuite/documentation/blob/master/Running-Mastodon/Production-guide.md)。これを含めた各種公式ドキュメントは[tootsuite/documentation](https://github.com/tootsuite/documentation)からたどることができる。
...
まずDockerをインストールする。[ここ](https://docs.docker.com/engine/installation/linux/centos/)の手順通り。
...
あとは指示に従う。ただし、すでに`skoji.jp`の証明書を作ったあとにこれをやると`/etc/letsencrypt/live`などに、`skoji.jp-0001`というのができてしまう。今回は手動で書き換えたが、もっと良い手があるだろう。
...
このあと、mastodonユーザに`sudo su mastodon -s /bin/bash`などでなっておく。
...
`docker-compose.yml`を変更する。以下をコメントアウトして、DBとredisのデータを、`/opt/mastodon`以下に永続化する。
...
次のコマンドを3回実行し、それぞれの結果を入れる。（初回は時間がかかる）。
